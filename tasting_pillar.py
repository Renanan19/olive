# -*- coding: utf-8 -*-
"""
tasting_pillar.py -- build THE root-level "olive oil tasting" pillar (x4 langs).
"Olive oil tastings" is the 2nd topic Bing AI cites the site for. Deep, citable
hub with Article + HowTo (how to taste) + FAQPage + BreadcrumbList JSON-LD.
Venice-styled via seo.css. Same structure as sustainability_pillar.py.
"""
...
SLUGS = {"fr": "degustation-huile-olive", "en": "olive-oil-tasting",
         "it": "degustazione-olio-oliva", "el": "geusignosia-elaioladou"}

# Sustainability pillar slugs (for cross-pillar link)
SUST = {"fr": "huile-olive-durable", "en": "sustainable-olive-oil",
...
UI = {
    "fr": dict(idx="index-fr.html", nav="L'OR VERT", home="Accueil", crumb="Dégustation",
               ...
               sustlbl="Huile d'olive durable", also="Ressources liées", toc="Sommaire",
               updated="Mis à jour", read="lecture",
               foot="Le carnet de référence de L'Or Vert sur l'huile d'olive, la cuisine méditerranéenne et les usages du quotidien."),
    "en": dict(idx="index-en.html", nav="L'OR VERT", home="Home", crumb="Tasting",
               ...
               notes="blog/notes-degustation-huile-olive-en.html", noteslbl="Tasting notes",
               sustlbl="Sustainable olive oil", also="Related resources", toc="Contents",
               updated="Updated", read="read",
               foot="L'Or Vert's reference notebook on olive oil, Mediterranean cooking and everyday uses."),
    "it": dict(idx="index-it.html", nav="L'OR VERT", home="Home", crumb="Degustazione",
               ...
               notes="blog/notes-degustation-huile-olive-it.html", noteslbl="Note di degustazione",
               sustlbl="Olio d'oliva sostenibile", also="Risorse collegate", toc="Indice",
               updated="Aggiornato", read="lettura",
               foot="Il taccuino di riferimento di L'Or Vert su olio d'oliva, cucina mediterranea e usi quotidiani."),
    "el": dict(idx="index-el.html", nav="L'OR VERT", home="Αρχική", crumb="Γευσιγνωσία",
               ...
               notes="blog/notes-degustation-huile-olive-el.html", noteslbl="Σημειώσεις γευσιγνωσίας",
               sustlbl="Βιώσιμο ελαιόλαδο", also="Σχετικές πηγές", toc="Περιεχόμενα",
               updated="Ενημερώθηκε", read="ανάγνωση",
...
def render(lang, d):
    ...
    article = {
        ...
        "inLanguage": lang, "articleSection": "Tasting", "image": OG,
    ...
    howto = {
        ...
        "supply": [{"@type": "HowToSupply", "name": n} for n in d["howto_supply"]],
...
<main class="container">
    <article class="guide pillar-guide">
        ...
        <div class="pillar-linkbox">
            ...
            <a href="{ui['notes']}">{esc(ui['noteslbl'])}</a>
            <a href="{ui['obs']}">{esc(ui['obslbl'])}</a>
            <a href="{SUST[lang]}.html">{esc(ui['sustlbl'])}</a>
